...
bool visited[mazeSize][mazeSize] = {}; // bool은 초기화하면 false
...
int main()
{
	...
	while (!locationStack.IsEmpty())
	{
		...
		// 현재 위치에서 상화좌우의 이동 가능한 위치를 스택에 저장
		for (int ix = 0; ix < 4; ++ix)
		{
			...
			if (IsValidLocation(nextRow, nextCol))
			{
				...
				{
					// 스택에 추가한 위치는 바로 방문 처리
					// 다음 번에 이동 가능한 위치 탐색할 때 중복 방지를 위함
